...
public class UserController {
    ...
    public ResponseEntity<?> updateUser(HttpServletRequest request, @RequestBody User user) {
        ...
        Map<String, String> response = new HashMap<>();
        user1.setId(userService.findByUsername(user.getName()).getId());
    ...
    public ResponseEntity<?> getUserBudget(@PathVariable int id) {
        double budget = userService.getBudgetByUserId(id);

        if (budget == 0.0) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                                 .body("Utente non trovato o budget non impostato.");
        }

        return ResponseEntity.ok(Map.of("weekly_budget", budget));
    }
}
