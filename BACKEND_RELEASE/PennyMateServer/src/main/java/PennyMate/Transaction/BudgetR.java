...
import PennyMate.Entity.BudgetPeriodDays;
...
public class BudgetR {
    ...
    public Budget getBudget(int userId) {
        ...
        return jdbcTemplate.queryForObject(sql, new Object[]{userId}, (rs, rowNum) -> {
            ...
            BigDecimal week = rs.getBigDecimal("week");
            BigDecimal month = rs.getBigDecimal("month");
            BigDecimal year = rs.getBigDecimal("year");
    ...
    public BudgetPeriodDays getPeriodDaysWithTransactions(int userId) {
        String sql = """
            SELECT
  COUNT(DISTINCT CASE
    WHEN t.type = FALSE AND t.date BETWEEN b.wDate AND DATE_ADD(b.wDate, INTERVAL (7 - WEEKDAY(b.wDate)) DAY)
    THEN t.date
  END) AS weeklyDays,

  COUNT(DISTINCT CASE
    WHEN t.type = FALSE AND t.date BETWEEN b.mDate AND LAST_DAY(b.mDate)
    THEN t.date
  END) AS monthlyDays,

  COUNT(DISTINCT CASE
    WHEN t.type = FALSE AND t.date BETWEEN b.yDate AND MAKEDATE(YEAR(b.yDate), 1) + INTERVAL 1 YEAR - INTERVAL 1 DAY
    THEN t.date
  END) AS yearlyDays
FROM transactions t
...
WHERE t.user_id = ?
        ...
        return jdbcTemplate.queryForObject(sql, new Object[]{userId}, (rs, rowNum) -> {
        	BudgetPeriodDays result = new BudgetPeriodDays();
            result.setWeeklyDays(rs.getInt("weeklyDays"));
            result.setMonthlyDays(rs.getInt("monthlyDays"));
            result.setYearlyDays(rs.getInt("yearlyDays"));
            return result;
        });
    }
