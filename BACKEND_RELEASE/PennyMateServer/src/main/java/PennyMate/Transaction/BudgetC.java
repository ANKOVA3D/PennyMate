...
public class BudgetC {
    ...
    public ResponseEntity<?> getBudgetPeriodDays(HttpServletRequest request) {
        User user = (User) request.getSession().getAttribute("user");
        ...
        try {
            BudgetPeriodDays days = budgetRepository.getPeriodDaysWithTransactions(user.getId());
            return ResponseEntity.ok(days);
        } catch (Exception e) {
        	System.out.println(e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Errore durante il calcolo dei giorni");
        }
    }
